...
- authserver is reponsible for maintaining user details, authentication and authorization, issuing and validating JWT tokens etc 
- user can sign-up, login, change passwords. It handles forgot password flows, email verification flows etc. - When user logs in user details are **cached to redis** And cache will get updated when there are any changes to the user details.
...
- it's the main entry point to the underlying microservices. All requests pass throught the gatewayserver.
- Gatewayserver intercepts every http requests and performs necessary checks such as 
  - authentication (using authserver or keycloak) and role based authorization.
...
**4. cart-service**
- cart service is responsible to maintain the items in the cart and place an order. Users can add items to the cart, can change the quantity, apply coupons etc, it then calls order service to place the order.
- items in the cart are deleted as soon as the order is placed
...
- main funtions are:
  - to send email notifications. diff services -> notification-service
  - to create basic profile when user signs up. authserver -> profile-service
  - to update the stock, order-service -> catalog service
...
- refers to repo's atozmart-config directory to resolve all the clients configuration based on application name.

**13. eurekasever**
- eurekasever acts as service registry allowing the clients to perform service discovery

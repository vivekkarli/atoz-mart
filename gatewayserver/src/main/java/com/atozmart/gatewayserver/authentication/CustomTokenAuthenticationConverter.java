...
import com.atozmart.gatewayserver.exception.GatewayException;
	...
	private Mono<Authentication> validateAtozmartToken(String token) {
		return webClient.get().uri(atozmartAuthorizeUrl + "?token=" + token)
				...
				.onStatus(HttpStatusCode::is4xxClientError,
						response -> Mono.error(new GatewayException("Invalid atozmart token", HttpStatus.UNAUTHORIZED)))
				.onStatus(HttpStatusCode::is5xxServerError,
						response -> Mono.error(
								new GatewayException("server error at atozmart", HttpStatus.INTERNAL_SERVER_ERROR)))
